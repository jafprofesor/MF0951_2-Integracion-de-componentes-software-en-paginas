document.addEventListener("DOMContentLoaded", function () {
  const formulario = document.getElementById("formulario-mixto");
  const resultado = document.getElementById("resultado");
  const spinner = document.getElementById("spinner");
  const emailInput = document.getElementById("email");
  const verificandoEmail = document.getElementById("verificando-email");
  const passwordInput = document.getElementById("password");

  // Variables para controlar la validación del email
  let timeoutId;
  ...
  emailInput.addEventListener("input", function () {
    ...
    verificandoEmail.style.display = "none";

    // Validar formato de email
    if (validarEmail(this.value)) {
      // Si el formato es válido, verificar disponibilidad después de un tiempo
      timeoutId = setTimeout(() => {
        verificarDisponibilidadEmail(this.value);
      }, 500);
    } else {
  ...
  function validarCampo(campo) {
    ...
    switch (campo.id) {
      ...
      case "email":
        // La validación del email se maneja en el evento input
        if (!campo.value.trim()) {
          ...
          // No marcar como válido si no se ha verificado la disponibilidad
          esValido = false;
        }
        break;
  ...
  function verificarDisponibilidadEmail(email) {
    ...
    verificandoEmail.style.display = "block";
    ...
    fetch("/api/verificar-email", {
      ...
      .then((data) => {
        ...
        if (data.existe) {
          // Email ya registrado
          mostrarError("email", data.message);
          emailValido = false;
        } else {
          // Email disponible
      ...
      .catch((error) => {
        ...
        // Mostrar error
        mostrarError("email", "Error al verificar el email");
        emailValido = false;
      });
  }
